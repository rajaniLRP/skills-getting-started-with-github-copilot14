    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<ul class="participants-list">${details.participants.map(email => `
              <li>
                ...
                <button class="participant-delete" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">✕</button>
              </li>
            `).join("")}</ul>`
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants-header"><span class="participants-label"><strong>Participants:</strong></span></div>
          ${participantsHtml}
        `;
